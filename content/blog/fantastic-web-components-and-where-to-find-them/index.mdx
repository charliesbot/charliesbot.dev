...
Then, a workmate sees that component. That solves a problem her team has! When she tried to use it, she discovers a huge blocker.
...
Most of the teams that I worked with built a component library to reuse everywhere. Funny thing is, each team redo this library according to the tech stack they are using.
...
Web components are *kinda* a new standard for the web.
...
Isn't weird that there are libraries that cover each framework? React, Angular, Vue, you name it.
...
Competition is great (and necessary!). But that's great if those implementations could be used no matter the tech stack. We are talking about basic components, that shouldn't know the complexity of our app. Let's make that dream true.
...
This lets us create new elements that encapsulate our custom component.
...
When the web component arrives at the DOM, the shadow DOM has not been attached to the tag.
You may see that they flash while they're getting the resources and the styles.

This callback is triggered when the whole component is ready, so we can start doing our own magic, like reading attributes, for example.

# Tradeoffs

We may agree that the API is a little bit verbose. That's probably the main throwback if you are used to React or Vue, where defining a component and its props are easier and more declarative.

Fortunately, other developers are stepping up with solutions like Stencil or Svelte.
...
Of course, I'm not implying we should stop using frameworks. React/Vue/Angular do way more than just components. They offer an all-in-one solution
and a lot of improvements and helpers that are extremely valuable to deliver solutions.

But when talking about the core bones of our application, web components make total sense:
