...
Estew post se enfocará en la segunda pregunta, la cual también es conocida como *Big-O notation*.
...
##### O(n) - Tiempo Linear
Un algoritmo tiene complejidad O(n) cuando su performance crece de forma linear según el set de datos.

##### O(nlogn) - Tiempo Linealitmico
Cuando un algoritmo corre una función logaritmica por operación linear, podemos decir que su complejidad es O(nlogn).
...
Este algoritmo también recorre la lista de sumas para obtener el valor más alto. En cada iteración, reemplaza una variable para estar al tanto de este valor máximo. Su complejidad es linear.
